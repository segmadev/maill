...
import { AlertCircle, CheckCircle2, Copy, ExternalLink, RefreshCw, Lock, Eye, EyeOff, Clock, Loader } from 'lucide-react'
...
export default function OAuthAuthorizationFlow({ open, onClose, onSuccess }) {
  ...
  const availableScopes = [
    { value: 'Mail.Read', label: 'Mail.Read - Read emails', checked: true },
    { value: 'Mail.Send', label: 'Mail.Send - Send emails', checked: true },
    { value: 'Mail.ReadWrite', label: 'Mail.ReadWrite - Read & write emails', checked: true },
    { value: 'offline_access', label: 'offline_access - Refresh token', checked: true },
    { value: 'User.Read', label: 'User.Read - Read profile', checked: false },
  ]
  ...
  const handleOpenUrl = () => {
    ...
    // Start polling to detect when account is added
    setStep('waiting')
    startPolling()
  }
  ...
  const startPolling = () => {
    ...
    pollingIntervalRef.current = setInterval(async () => {
      ...
      try {
        ...
        const response = await fetch('http://localhost:8765/api/admin/accounts', {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('admin-auth')?.split('token":"')[1]?.split('"')[0] || ''}`,
            'Content-Type': 'application/json',
          },
        })
        const result = await response.json()

        // Look for account matching the email we're adding
        if (result.data && Array.isArray(result.data)) {
          const newAccount = result.data.find(acc => acc.email === form.email)
  ...
  return (
    <Modal open={open} onClose={onClose} title="Add Account - OAuth Authorization Flow">
        ...
        {step === 'form' && (
          <form onSubmit={handleGenerateUrl} className="space-y-4">
            <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-3 text-sm text-yellow-300">
              <p className="font-semibold mb-1">⚠️ Verify Your Credentials</p>
              <p className="text-xs">Make sure your Client ID and Secret match exactly what's in Azure:</p>
              <ol className="mt-2 ml-4 space-y-0.5 text-xs list-decimal">
            ...
            <div>
              ...
              <div className="space-y-2 bg-surface-raised rounded-lg p-3 border border-surface-border max-h-40 overflow-y-auto">
              ...
              <p className="text-xs text-gray-500 mt-1">Select which permissions to request from Microsoft</p>
            </div>
          ...
          <div className="space-y-4">
            <div className="flex flex-col items-center gap-4 p-6">
              ...
              <div className="text-center">
                <h3 className="text-lg font-semibold text-white mb-2">Waiting for Authorization...</h3>
                <p className="text-gray-400">Complete the login process in the popup window.</p>
                <p className="text-sm text-gray-500 mt-2">This modal will automatically update when done.</p>
              </div>
            </div>

            <div className="p-4 bg-blue-500/10 border border-blue-500/20 rounded-lg text-sm text-blue-300">
              <p className="font-semibold mb-2">✓ In the popup window:</p>
              <ol className="space-y-1 ml-4 list-decimal">
                <li>Sign in with your Outlook account</li>
                <li>Click "Accept" to authorize</li>
                <li>You'll see a success page</li>
              </ol>
            </div>
